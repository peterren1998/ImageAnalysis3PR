...
class Cell_List():
    ...
    def _update_cell_segmentations(self, _cell_coord_fl='cell_coords.pkl',
        ...
        if not os.path.exists(_cell_coord_savefile):
            raise IOError(f'{_cell_coord_savefile} does not exist, exit')
        # open cell_coord.pkl
    ...
    def _create_cells_fov(self, _fov_ids, _num_threads=None, _sequential_mode=False, _plot_segmentation=True, 
        ...
        _args = []
        for _fov_id in _fov_ids:
            self.log_and_print(f'+ Load segmentation for fov {_fov_id}', verbose=_verbose)
            self.log_and_print(f'+ Loading segmentation from {os.path.join(_dapi_fd, self.fovs[_fov_id])}', verbose=_verbose)
    ...
    def _crop_image_for_cells(self, _data_type='unique', _load_in_ram=False, 
                              ...
                              _force=False, _overwrite_cell_info=False, 
                              skip_corrections=True,
                              plot_segmentation=False, _verbose=True):
    ...
    def _update_chromosomes_for_cells(self, _coord_filename='chrom_coords.pkl', 
        ...
        if len(_coord_list) > len(self.cells):
            raise ValueError(f'Number of cells does not match between cell-list and {_chrom_savefile}')
...
class Cell_Data():
    ...
    def _load_segmentation(self, _min_shape_ratio=0.035, _signal_cap_ratio=0.2, _denoise_window=5,
                           _shrink_percent=15, _max_conv_th=0, _min_boundary_th=0.48, seg_prefix='segmentation_label_',
                           _load_in_ram=True, _save=True, _force=False, _verbose=False, logger=None):
        # check attributes
        if not hasattr(self, 'channels') or not hasattr(self, 'color_dic'):
            self._load_color_info()
        # find the folder name for dapi
        ...
        if not _select_dapi:
            raise ValueError("No DAPI folder detected in annotated_folders, stop!")
        log_and_print(f'+++ loading segmentation file for cell {self.cell_id} using DAPI file {os.path.join(_dapi_fd, self.fovs[self.fov_id])}...',
        ...
        _segmentation_labels = visual_tools.DAPI_convoluted_segmentation(
            ...
            denoise_window=_denoise_window, shrink_percent=_shrink_percent,
            max_conv_th=_max_conv_th, min_boundary_th=_min_boundary_th,
            make_plot=False, return_images=False, seg_prefix=seg_prefix,
            double_seg_size=self.shared_parameters['double_seg_size'], return_mode_proj_if_3D=True,
        ...
        if not hasattr(self, 'cell_id'):
            ...
            raise ValueError('segmentation label does not contain this cell id:', self.cell_id)
    ...
    def _crop_images(self, _data_type, _num_threads=12, 
                     ...
                     skip_corrections=False, plot_segmentation=False,
                     _verbose=True):
        ...
        if not hasattr(self, 'segmentation_label'):
            self._load_segmentation(_verbose=True)
            if plot_segmentation:
                plt.figure()
                plt.imshow(self.segmentation_label)
                plt.colorbar()
                plt.title(f'Segmentation result for fov:{self.fov_id}')
                plt.show()
        if not hasattr(self, 'drift'):
            self._load_drift()
        # check type
    ...
    def _generate_chromosome_image(self, _source='unique', _max_count=90, _verbose=False):
        ...
        if _source == 'combo':
            if not hasattr(self, 'combo_groups'):
                ...
                print('-- cell_data does not have combo images, trying to load now.')
            ...
            if not hasattr(self, _im_attr) or not hasattr(self, _id_attr):
                ...
                print(f'-- cell:{self.cell_id} in fov:{self.fov_id} does not have {_im_attr}, trying to load now.')
    ...
    def _pick_chromosome_manual(self, _save_folder=None, _save_fl='chrom_coord.pkl'):
        ...
        if not hasattr(self, 'chrom_coords'):
            raise ValueError("chromosome coordinates does not exist in attributes.")
        _coord_dic = {'coords': [np.flipud(_coord) for _coord in self.chrom_coords],
                      'class_ids': list(np.zeros(len(self.chrom_coords), dtype=np.uint16)),
                      'pfits':{},
                      'dec_text':{},
                      }
        #pickle.dump(_coord_dic, open(_chrom_savefile, 'wb'))
    ...
    def _multi_fitting_for_chromosome(self, _data_type='unique', _decoded_flag='diff', 
        ...
        if hasattr(self, _spot_attr) and not _overwrite \
            ...
            if _data_type in self.shared_parameters['allowed_data_types']:
                _seeding_args = (_max_seed_count, _fit_window, _gfilt_size, _background_gfilt_size, _max_filt_size, _seed_by_per, _th_seed_percentile, 150, None, True, 10, _min_seed_count, 0, False)
